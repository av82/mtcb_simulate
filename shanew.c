...
void SHAUpdate(SHA_CTX *shsInfo, BYTE *buffer, int count)
{
    ...
    /* Process data in SHS_DATASIZE chunks     while( count >= SHS_DATASIZE )
        {
...
unsigned char* shafunction(unsigned char *message,unsigned int len,unsigned char*output)
{
	SHA_CTX sha;
	int i;
unsigned char BUFFER[40];
	SHAInit(&sha);
	SHAUpdate(&sha, message,len);
	SHAFinal(digest, &sha);
