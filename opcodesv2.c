...
typedef long long int byte_8; \\64 bits 
struct timespec tp;
...
{
	\\flag_loaded is set to one upon hmac verification for 3 leaves incase of syn state else for only 1; only after which parse is done.
	
	int sub_id;	 clock_gettime(CLOCK_REALTIME, &tp);
\\ for nano seconds tp.tv_nsec
	byte_8  CurrentTickCount = (byte_8)tp.tv_sec;
   \\ to save result of OP_2 to operate on OP_3 
